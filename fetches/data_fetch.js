    ...
    const timelinePromises = matches.map(({ tournamentId, matchId }) => 
        limit(async () => {
            const timeline = await fetchData(
                `${baseUrl}/${tournamentId}/matches/${matchId}/timeline?eventTypes=Shot,Pass,BallContest,Dribble,HeaderContest,ThrowIn,CornerKick,KickOff,GoalKick,FreeKick,PenaltyKick&pageSize=1000&normalize=true`
            );
